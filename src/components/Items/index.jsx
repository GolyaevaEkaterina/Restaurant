...
const Items = ({items, setCartItems, cartItems, restaurants}) => {    
    ...
    const [isOpenContainerDeleteCart, setIsOpenContainerDeleteCart] = useState(false)
    ...
    return(
        ...
        {items.map((i) => {
           ...
           if(itemCoincidence===undefined){
            return(            
              <div className="p-5 bg-white rounded-3xl shadow-md flex flex-col justify-between">
                <img alt='' src={i.image} className='h-44 w-full object-cover object-center rounded-3xl'/>
                <p className="text-left text-lg mb-4">{i.name}</p>
                <div>
                  <p className="text-left text-2xl mb-1 ml-1">{Math.round(i.price)} р.</p>
                  <button onClick={()=>addItem(i)} className="px-6 py-2 bg-zinc-100 rounded-md w-full text-lg">Добавить</button>
                </div>                
              </div>             
            )
           }
              ...
              <div className="p-5 bg-white rounded-3xl shadow-md flex flex-col justify-between">
                ...
                <div>
                  ...
                  <div className="flex justify-between align-center gap-6 px-6 py-2 bg-zinc-100 rounded-md w-full text-lg">
                    <button className="pt-1" onClick={()=>deleteItem(i)}><i class="fa-solid fa-minus"></i></button>
                    <p>{itemCoincidence.quantity}</p>  
                    <button onClick={()=>addItem(i)}><i class="fa-solid fa-plus"></i></button>                    
                  </div>
                </div>
              </div>
            )
          ...
          <ContainerDeleteCart isOpenContainerDeleteCart={isOpenContainerDeleteCart} setIsOpenContainerDeleteCart={setIsOpenContainerDeleteCart} deleteCart={deleteCart} nameRestСurrent={nameRestСurrent} nameRestPrevious={nameRestPrevious}/>
       </div>
    )
}
