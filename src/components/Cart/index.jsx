  ...
  return (
    <div>
      <div className="max-w-2xl w-full mx-auto mt-10 rounded-3xl bg-white p-4 md:p-10">
          ...
          return (
            <div className="flex justify-between gap-3 items-center max-w-2xl w-full mx-auto border-b-2 my-4">
              <img
                alt=""
                src={c.image}
                className="h-20 w-20 object-cover object-center rounded-3xl mb-2"
              />
              <p className="text-lg md:text-xl md:h-10 md:w-60 text-start">
                {c.name}
              </p>
              <div className="flex-col items-end">
                ...
                <div className="flex justify-between items-center gap-6 h-10 px-4 py-1 bg-zinc-100 rounded-md text-md">
                  <button className="pt-1" onClick={() => deleteItem(c)}>
                    <i class="fa-solid fa-minus"></i>
                  </button>
                  <p>{c.quantity}</p>
                  <button onClick={() => addItem(c)}>
        ...
        <div
          onClick={() => setIsOpenOrderContainer(true)}
          className={classNames(
            "cursor-pointer flex justify-between items-center px-2 md:px-10 bg-orange-400 rounded-md mt-6 h-10 text-slate-800 text-xl md:text-2xl font-semibold shadow-md",
            {
        ...
        <div>
          <OrderContainer
            setIsOpenOrderButton={setIsOpenOrderButton}
            setTextCart={setTextCart}
            cartItems={cartItems}
            setCartItems={setCartItems}
            setIsOpenOrderContainer={setIsOpenOrderContainer}
            isOpenOrderContainer={isOpenOrderContainer}
          />
        </div>
      </div>
    </div>
